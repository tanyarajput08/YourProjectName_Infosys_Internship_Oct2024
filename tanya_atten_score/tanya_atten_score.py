...
# Load the known image of Tanya Chauhan
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                if attentive == 'Yes':
                    cv.putText(frame, f'Attentive (Score: {attention_score:.2f})', (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv.LINE_AA)
                else:
                    cv.putText(frame, f'Not Attentive (Score: {attention_score:.2f})', (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2, cv.LINE_AA)
    ...
    if not df.empty:
        df.to_excel('attendance_with_attentionscore.xlsx', index=False)
        print("Attendance with attentiveness saved to 'attendance_with_attentionscore.xlsx'.")
